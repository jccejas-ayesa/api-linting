...
@Component
public class AsyncOperationDescriptionRule implements LintingRule {

    @Override
    public String getRuleId() {
        return "asyncapi-operation-description";
    }

    @Override
    public String getDescription() {
        return "Validates that AsyncAPI operations include descriptions";
    }
    ...
    public List<LintingIssue> apply(OpenAPI openAPI) {
        ...
        for (Map.Entry<String, PathItem> pathEntry : openAPI.getPaths().entrySet()) {
            ...
            for (Map.Entry<PathItem.HttpMethod, Operation> operationEntry : pathItem.readOperationsMap().entrySet()) {
                ...
                if (operation != null && (operation.getDescription() == null || operation.getDescription().isBlank())) {
                    issues.add(LintingIssue.warning(
                            getRuleId(),
                            pathEntry.getKey() + " [" + operationEntry.getKey().name() + "]",
                            "Operation description is missing",
                            "Add a non-blank description for the operation"
                    ));
                }
            }
        }
