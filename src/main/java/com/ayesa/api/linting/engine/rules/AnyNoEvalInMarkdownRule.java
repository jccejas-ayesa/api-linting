...
@Component
public class AnyNoEvalInMarkdownRule implements LintingRule {

    private static final Pattern BLOCKED_PATTERN = Pattern.compile("(?i)eval\\s*\\(");

    @Override
    public String getRuleId() {
        return "no-eval-in-markdown";
    }

    @Override
    public String getDescription() {
        return "Descriptions must not contain eval-like code";
    }
    ...
    private void inspectDescription(List<LintingIssue> issues, String location, String description) {
        if (description != null && BLOCKED_PATTERN.matcher(description).find()) {
            issues.add(LintingIssue.error(
                    getRuleId(),
                    location,
                    "Description contains eval-like code",
                    "Remove eval usage from Markdown and replace it with plain documentation"
            ));
        }
    }
