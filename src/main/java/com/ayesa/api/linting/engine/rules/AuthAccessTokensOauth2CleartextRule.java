...
@Component
public class AuthAccessTokensOauth2CleartextRule implements LintingRule {

    @Override
    public String getRuleId() {
        return "access-tokens-oauth2-cleartext";
    }

    @Override
    public String getDescription() {
        return "OAuth2 security schemes must only be used with HTTPS server URLs";
    }
    ...
    public List<LintingIssue> apply(OpenAPI openAPI) {
        ...
        List<String> oauth2Schemes = securitySchemes.entrySet().stream()
                .filter(entry -> entry.getValue() != null && entry.getValue().getType() == SecurityScheme.Type.OAUTH2)
        ...
        if (openAPI.getServers() == null || openAPI.getServers().isEmpty()) {
            for (String schemeName : oauth2Schemes) {
                issues.add(LintingIssue.error(getRuleId(),
                        "/components/securitySchemes/" + schemeName,
                        "OAuth2 security scheme '" + schemeName + "' is defined but no HTTPS server URLs are declared",
                        "Define one or more HTTPS server URLs when using OAuth2"));
            }
            return issues;
        }
        ...
        for (String schemeName : oauth2Schemes) {
            for (int i = 0; i < openAPI.getServers().size(); i++) {
                ...
                if (!isHttpsOrTemplated(url)) {
                    issues.add(LintingIssue.error(getRuleId(),
                            "/servers/" + i,
                            "OAuth2 security scheme '" + schemeName + "' requires HTTPS, but server URL is '" + url + "'",
