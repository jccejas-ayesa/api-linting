...
@Component
public class AuthApiKeysInQueryRule implements LintingRule {

    @Override
    public String getRuleId() {
        return "api-keys-in-query";
    }

    @Override
    public String getDescription() {
        return "API key security schemes must not use query parameters as the transport location";
    }
    ...
    public List<LintingIssue> apply(OpenAPI openAPI) {
        ...
        for (Map.Entry<String, SecurityScheme> entry : openAPI.getComponents().getSecuritySchemes().entrySet()) {
            ...
            if (scheme != null
                    ...
                    && scheme.getIn() == SecurityScheme.In.QUERY) {
                issues.add(LintingIssue.error(getRuleId(),
                        "/components/securitySchemes/" + entry.getKey(),
                        "API key security scheme '" + entry.getKey() + "' uses query parameters",
